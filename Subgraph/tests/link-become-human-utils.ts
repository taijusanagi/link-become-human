...
import {
  ...
  SeedUpdated,
  Transfer
} from "../generated/LinkBecomeHuman/LinkBecomeHuman"
...
export function createHumanityScoreUpdatedEvent(
  tokenId: BigInt,
  humanityScore: string
): HumanityScoreUpdated {
  let humanityScoreUpdatedEvent = changetype<HumanityScoreUpdated>(
    newMockEvent()
  )

  humanityScoreUpdatedEvent.parameters = new Array()

  humanityScoreUpdatedEvent.parameters.push(
    new ethereum.EventParam(
      "tokenId",
      ethereum.Value.fromUnsignedBigInt(tokenId)
    )
  ...
  humanityScoreUpdatedEvent.parameters.push(
    new ethereum.EventParam(
      "humanityScore",
      ethereum.Value.fromString(humanityScore)
    )
  )

  return humanityScoreUpdatedEvent
}
...
export function createSeedUpdatedEvent(
  tokenId: BigInt,
  seed: BigInt
): SeedUpdated {
  let seedUpdatedEvent = changetype<SeedUpdated>(newMockEvent())

  seedUpdatedEvent.parameters = new Array()

  seedUpdatedEvent.parameters.push(
    new ethereum.EventParam(
      "tokenId",
      ethereum.Value.fromUnsignedBigInt(tokenId)
    )
